  ...
  "cells": [
    ...
    },
    {
      ...
      "source": [
        ...
        "        \"\"\"Creates a pipeline for missing value imputation and scaling.\"\"\"\n",
        "        return Pipeline([\n",
        ...
        "        \"\"\"Ensure input dataframe has all required feature columns in correct order.\"\"\"\n",
        "        X = df.copy()\n",
        ...
        "        \"\"\"\n",
        "        Trains a calibrated multi-class RandomForest model.\n",
        "        train_df must include the 'fetal_health' column.\n",
        "        \"\"\"\n",
        "        if \"fetal_health\" not in train_df.columns:\n",
        "            raise ValueError(\"Training dataframe must include 'fetal_health' column\")\n",
        "\n",
        "        X_raw = self._align_features(train_df.drop(\"fetal_health\", axis=1))\n",
        "        y = train_df[\"fetal_health\"].astype(float)\n",
        "\n",
        ...
        "                    if cls_int in [1, 2, 3]:\n",
        "                        full[:, cls_int - 1] = proba[:, idx]\n",
        "                row_sums = full.sum(axis=1, keepdims=True)\n",
        "                row_sums[row_sums == 0] = 1.0\n",
        "                return full / row_sums\n",
        "            return np.full((proba.shape[0], 3), 1/3, dtype=float)\n",
        "\n",
        "        return proba\n",
        "\n",
        "    def predict(self, df):\n",
        "        \"\"\"Returns predicted class labels (1.0, 2.0, 3.0).\"\"\"\n",
        "        proba = self.predict_proba(df)\n",
        "        pred_idx = np.argmax(proba, axis=1)\n",
        "        return (pred_idx + 1).astype(float)\n"
      ]
    },
    {
      "cell_type": "code",
    ...
    },
    {
      ...
      "source": [
        ...
        "    print(f\"Target Accuracy: 0.85 (85%)\")\n",
        "    print(f\"Status: {'✓ PASSED' if test_accuracy >= 0.85 else '✗ FAILED'}\")\n",
        "    print(f\"{'='*60}\\n\")\n",
        "\n",

...
def test_predict_with_missing_values(trained_model):
    ...
    X = df_test.drop("fetal_health", axis=1).copy()
    if X.shape[1] > 0:
        X.iloc[0, 0] = np.nan
...
def test_predict_with_missing_column(trained_model):
    ...
    # Drop one column if possible
    if X.shape[1] > 1:
        drop_col = X.columns[0]
...
def test_probability_quality(trained_model):
    ...
    X = df_test.drop("fetal_health", axis=1)
...
def test_model_performance(trained_model):
    ...
    # Get true labels
    y_true = df_test["fetal_health"].astype(float)
    ...
    assert accuracy >= 0.90, f"Accuracy too low: {accuracy:.4f} < 0.90"
    assert macro_f1 >= 0.88, f"Macro F1 too low: {macro_f1:.4f} < 0.88"
    assert recall_class3 >= 0.75, f"Class-3 recall too low: {recall_class3:.4f} < 0.75"
